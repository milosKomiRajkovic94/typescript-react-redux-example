...
export const createUserEvent = (): ThunkAction<
  ...
  RootState, 
  undefined, 
  CreateRequestAction | CreateSuccessAction | CreateFailureAction>  => async (dispatch, getState) => {
    dispatch({
      type: CREATE_REQUEST 
    });
...
export const deleteUserEvent = (id: UserEvent['id']): ThunkAction<
  ...
  RootState, 
  undefined, 
  DeleteRequestAction | DeleteSuccessAction | DeleteFailureAction> => async(dispatch, ) => {
    dispatch({
      type: DELETE_REQUEST
    });
...
const updateUserEvent = (event: UserEvent): ThunkAction<
  ...
  UpdateRequestAction | UpdateSuccessAction | UpdateFailureAction> => async(dispatch) => {
    ...
    try{
      const response = fetch(`http://localhost:3001/events/${event.id}`, {
        method: 'PUT',
        headers:{
          'Content-Type': 'application/json' 
        },
      ...
      const updatedEvent: UserEvent = await (await response).json();
